...
export class WardleyPositioner {
	...
	private positionByEvolution(components: ComponentNode[]): void {
		for (const comp of components) {
			const stage = comp.attrs.strategic?.evolution_stage || 'custom';
	 ...
	 */
	private longestPathFromRoot(components: ComponentNode[], edges: ComponentEdge[]): Map<string, number> {
		...
		const roots: string[] = [];
		for (const comp of components) {
			const isUserNeed = comp.attrs.strategic?.type === 'user_need';
